# Task Manager API# Task Manager API

Java · Spring Boot · JWT · Redis · PostgreSQL · Docker · AWS SES · Google OAuthJava · Spring Boot · JWT · Redis · PostgreSQL · Docker · AWS SES

---

## Setup## Setup

**1 - Install Docker\*\***1 - Install Docker\*\*

https://www.docker.comhttps://www.docker.com

**2 - Configure environment\*\***2 - Run the application\*\*

Copy the example configuration and fill in your values:Open a terminal in the project root and run:

`bash`bash

cp src/main/resources/application-example.properties src/main/resources/application.propertiesdocker-compose up -d

`````



Edit `application.properties` with your credentials (JWT secret, AWS SES keys, Google Client ID).After running, the following services will be available:



**3 - Run the application**| Service    | Port | URL                                   |

| ---------- | ---- | ------------------------------------- |

```bash| API        | 8080 | http://localhost:8080                 |

docker-compose up -d| Swagger UI | 8080 | http://localhost:8080/swagger-ui.html |

```| PostgreSQL | 5432 | localhost:5432                        |

| Redis      | 6379 | localhost:6379                        |

| Service    | Port | URL                                   |

| ---------- | ---- | ------------------------------------- |**PostgreSQL connection:**

| API        | 8080 | http://localhost:8080                 |

| Swagger UI | 8080 | http://localhost:8080/swagger-ui.html |- Host: `localhost`

| PostgreSQL | 5432 | localhost:5432                        |- Port: `5432`

| Redis      | 6379 | localhost:6379                        |- Database: `taskmanager`

- Username: `admin`

**PostgreSQL connection:**- Password: `admin`



- Host: `localhost`**3 - Configure AWS SES (email sending)**

- Port: `5432`

- Database: `taskmanager`Open `src/main/resources/application.properties` and fill in your AWS credentials:

- Username: `admin`

- Password: `admin````properties

aws.ses.access-key=your-access-key

**4 - Configure AWS SES (email sending)**aws.ses.secret-key=your-secret-key

aws.ses.region=us-east-1

1. Create an IAM user at https://console.aws.amazon.com/iam with policy `AmazonSESFullAccess`aws.ses.from=your-verified-email@example.com

2. Generate an access key under **Security credentials → Create access key**```

3. Verify your sender email at https://console.aws.amazon.com/ses → **Verified Identities**

4. Fill in `aws.ses.*` properties in `application.properties`To set up AWS SES from scratch:



**5 - Configure Google OAuth (optional)**1. Create an IAM user at https://console.aws.amazon.com/iam with policy `AmazonSESFullAccess`

2. Generate an access key under **Security credentials → Create access key**

1. Go to https://console.cloud.google.com/apis/credentials3. Verify your sender email at https://console.aws.amazon.com/ses → **Verified Identities**

2. Create an OAuth 2.0 Client ID (Web application)

3. Copy the Client ID and fill in `google.client-id` in `application.properties`---



---## Making requests



## Making requestsOpen Swagger UI at http://localhost:8080/swagger-ui.html or use any HTTP client.



Open Swagger UI at http://localhost:8080/swagger-ui.html or use any HTTP client.All protected endpoints require the header:



All protected endpoints require the header:```

Authorization: Bearer <access_token>

`````

Authorization: Bearer <access_token>

```````---



---## Authentication flow



## Authentication flow**1 - Register an account**



### Standard registration```

POST > http://localhost:8080/auth/register

**1 - Register an account**```



``````json

POST /auth/register{

```  "name": "John Doe",

  "username": "johndoe",

```json  "email": "john@example.com",

{  "password": "secret123",

  "name": "John Doe",  "confirmPassword": "secret123"

  "username": "johndoe",}

  "email": "john@example.com",```

  "password": "secret123",

  "confirmPassword": "secret123"Response `201 Created`:

}

``````json

{

Response `201 Created`:  "message": "Registration successful! Please check your email to verify your account."

}

```json```

{

  "message": "Registration successful! Please check your email to verify your account."A verification email will be sent to the provided address.

}

```**2 - Verify your email**



**2 - Verify your email**```

POST > http://localhost:8080/auth/verify-email

```````

POST /auth/verify-email

````json

{ "token": "<token_from_email>" }

```json```

{ "token": "<token_from_email>" }

```Response `200 OK`:



Response `200 OK`:```json

{ "message": "Email verified successfully! You can now log in." }

```json```

{ "message": "Email verified successfully! You can now log in." }

```**3 - Login**



**3 - Login**```

POST > http://localhost:8080/auth/login

````

POST /auth/login

````json

{ "emailOrUsername": "johndoe", "password": "secret123" }

```json```

{ "emailOrUsername": "johndoe", "password": "secret123" }

```Response `200 OK`:



Response `200 OK`:```json

{

```json  "name": "John Doe",

{  "accessToken": "<access_token>",

  "name": "John Doe",  "refreshToken": "<refresh_token>"

  "accessToken": "<access_token>",}

  "refreshToken": "<refresh_token>"```

}

```Access token expires in **4 hours**. Use the refresh token to get a new one:



Access token expires in **4 hours**. Use the refresh token to get a new one:```

POST > http://localhost:8080/auth/refresh

````

POST /auth/refresh

````json

{ "refreshToken": "<refresh_token>" }

```json```

{ "refreshToken": "<refresh_token>" }

```---



### Google OAuth login## Endpoints



```### Auth

POST /auth/google

```| Method | Path                      | Auth | Status (success) | Description                                                |

| ------ | ------------------------- | ---- | ---------------- | ---------------------------------------------------------- |

```json| POST   | /auth/register            | No   | 201              | Create account and send verification email                 |

{ "token": "<google_id_token>" }| POST   | /auth/verify-email        | No   | 200              | Verify email with token from inbox                         |

```| POST   | /auth/resend-verification | No   | 200              | Resend verification email                                  |

| POST   | /auth/login               | No   | 200              | Login, returns access + refresh token                      |

Response `200 OK`:| POST   | /auth/refresh             | No   | 200              | Get new access token using refresh token                   |

| POST   | /auth/logout              | Yes  | 200              | Invalidate current session (requires refreshToken in body) |

```json| POST   | /auth/logout-all          | Yes  | 200              | Invalidate all sessions                                    |

{| POST   | /auth/forgot-password     | No   | 200              | Send password reset email                                  |

  "name": "John Doe",| POST   | /auth/reset-password      | No   | 200              | Reset password with token                                  |

  "accessToken": "<access_token>",

  "refreshToken": "<refresh_token>"### Users

}

```| Method | Path        | Auth | Status (success) | Description                     |

| ------ | ----------- | ---- | ---------------- | ------------------------------- |

If the user does not exist, a new account is created automatically with email already verified. If a user registered with email/password tries to log in via Google with the same email, they will receive a `USE_GOOGLE_LOGIN` or `INVALID_CREDENTIALS` error depending on direction.| GET    | /users/me   | Yes  | 200              | Get current authenticated user  |

| PUT    | /users/me   | Yes  | 200              | Update profile (name, username) |

---| DELETE | /users/me   | Yes  | 204              | Delete own account              |

| GET    | /users/{id} | Yes  | 200              | Get user by ID                  |

## Endpoints

### Workspaces

### Auth

| Method | Path             | Auth | Status (success) | Description                          |

| Method | Path                      | Auth | Status | Description                                                || ------ | ---------------- | ---- | ---------------- | ------------------------------------ |

| ------ | ------------------------- | ---- | ------ | ---------------------------------------------------------- || POST   | /workspaces      | Yes  | 201              | Create workspace                     |

| POST   | /auth/register            | No   | 201    | Create account and send verification email                 || GET    | /workspaces      | Yes  | 200              | List all workspaces                  |

| POST   | /auth/verify-email        | No   | 200    | Verify email with token from inbox                         || GET    | /workspaces/{id} | Yes  | 200              | Get workspace with boards            |

| POST   | /auth/resend-verification | No   | 200    | Resend verification email                                  || PUT    | /workspaces/{id} | Yes  | 200              | Update workspace                     |

| POST   | /auth/login               | No   | 200    | Login, returns access + refresh token                      || DELETE | /workspaces/{id} | Yes  | 204              | Delete workspace and all its content |

| POST   | /auth/google              | No   | 200    | Login/register with Google ID token                        |

| POST   | /auth/refresh             | No   | 200    | Get new access token using refresh token                   |### Workspace Members

| POST   | /auth/logout              | Yes  | 200    | Invalidate current session (requires refreshToken in body) |

| POST   | /auth/logout-all          | Yes  | 200    | Invalidate all sessions                                    || Method | Path                              | Auth | Status (success) | Description                      |

| POST   | /auth/forgot-password     | No   | 200    | Send password reset email                                  || ------ | --------------------------------- | ---- | ---------------- | -------------------------------- |

| POST   | /auth/reset-password      | No   | 200    | Reset password with token                                  || GET    | /workspaces/{id}/members          | Yes  | 200              | List all members                 |

| POST   | /workspaces/{id}/members          | Yes  | 201              | Invite user by email or username |

### Users| DELETE | /workspaces/{id}/members/{userId} | Yes  | 200              | Remove member from workspace     |



| Method | Path        | Auth | Status | Description                     |### Boards

| ------ | ----------- | ---- | ------ | ------------------------------- |

| GET    | /users/me   | Yes  | 200    | Get current authenticated user  || Method | Path                     | Auth | Status (success) | Description                      |

| PUT    | /users/me   | Yes  | 200    | Update profile (name, username) || ------ | ------------------------ | ---- | ---------------- | -------------------------------- |

| DELETE | /users/me   | Yes  | 204    | Delete own account              || POST   | /boards?workspaceId={id} | Yes  | 201              | Create board in workspace        |

| GET    | /users/{id} | Yes  | 200    | Get user by ID                  || GET    | /boards?workspaceId={id} | Yes  | 200              | List boards from workspace       |

| GET    | /boards/{id}             | Yes  | 200              | Get board with lists and cards   |

### Workspaces| PUT    | /boards/{id}             | Yes  | 200              | Update board                     |

| DELETE | /boards/{id}             | Yes  | 204              | Delete board and all its content |

| Method | Path             | Auth | Status | Description                          |

| ------ | ---------------- | ---- | ------ | ------------------------------------ |### Board Members

| POST   | /workspaces      | Yes  | 201    | Create workspace                     |

| GET    | /workspaces      | Yes  | 200    | List all workspaces                  || Method | Path                          | Auth | Status (success) | Description                           |

| GET    | /workspaces/{id} | Yes  | 200    | Get workspace with boards            || ------ | ----------------------------- | ---- | ---------------- | ------------------------------------- |

| PUT    | /workspaces/{id} | Yes  | 200    | Update workspace                     || GET    | /boards/{id}/members          | Yes  | 200              | List all members                      |

| DELETE | /workspaces/{id} | Yes  | 204    | Delete workspace and all its content || POST   | /boards/{id}/members          | Yes  | 201              | Invite workspace member to this board |

| DELETE | /boards/{id}/members/{userId} | Yes  | 200              | Remove member from board              |

### Workspace Members

### Lists

| Method | Path                              | Auth | Status | Description                      |

| ------ | --------------------------------- | ---- | ------ | -------------------------------- || Method | Path                             | Auth | Status (success) | Description               |

| GET    | /workspaces/{id}/members          | Yes  | 200    | List all members                 || ------ | -------------------------------- | ---- | ---------------- | ------------------------- |

| POST   | /workspaces/{id}/members          | Yes  | 201    | Invite user by email or username || POST   | /boards/{boardId}/lists          | Yes  | 201              | Create list               |

| DELETE | /workspaces/{id}/members/{userId} | Yes  | 200    | Remove member from workspace     || GET    | /boards/{boardId}/lists          | Yes  | 200              | Get all lists             |

| GET    | /boards/{boardId}/lists/{listId} | Yes  | 200              | Get list                  |

### Boards| PUT    | /boards/{boardId}/lists/{listId} | Yes  | 200              | Update list               |

| DELETE | /boards/{boardId}/lists/{listId} | Yes  | 204              | Delete list and its cards |

| Method | Path                     | Auth | Status | Description                      |

| ------ | ------------------------ | ---- | ------ | -------------------------------- |### Cards

| POST   | /boards?workspaceId={id} | Yes  | 201    | Create board in workspace        |

| GET    | /boards?workspaceId={id} | Yes  | 200    | List boards from workspace       || Method | Path                                                 | Auth | Status (success) | Description                   |

| GET    | /boards/{id}             | Yes  | 200    | Get board with lists and cards   || ------ | ---------------------------------------------------- | ---- | ---------------- | ----------------------------- |

| PUT    | /boards/{id}             | Yes  | 200    | Update board                     || POST   | /boards/{boardId}/lists/{listId}/cards               | Yes  | 201              | Create card                   |

| DELETE | /boards/{id}             | Yes  | 204    | Delete board and all its content || GET    | /boards/{boardId}/lists/{listId}/cards               | Yes  | 200              | Get all cards                 |

| GET    | /boards/{boardId}/lists/{listId}/cards/{cardId}      | Yes  | 200              | Get card                      |

### Board Members| PUT    | /boards/{boardId}/lists/{listId}/cards/{cardId}      | Yes  | 200              | Update card                   |

| PATCH  | /boards/{boardId}/lists/{listId}/cards/{cardId}/move | Yes  | 200              | Move card to a different list |

| Method | Path                          | Auth | Status | Description                           || DELETE | /boards/{boardId}/lists/{listId}/cards/{cardId}      | Yes  | 204              | Delete card                   |

| ------ | ----------------------------- | ---- | ------ | ------------------------------------- |

| GET    | /boards/{id}/members          | Yes  | 200    | List all board members                |Card status values: `ACTIVE`, `ARCHIVED`, `COMPLETED`.

| POST   | /boards/{id}/members          | Yes  | 201    | Invite workspace member to this board |

| DELETE | /boards/{id}/members/{userId} | Yes  | 200    | Remove member from board              |---



### Lists## Usage example



| Method | Path                             | Auth | Status | Description               |**Create a workspace**

| ------ | -------------------------------- | ---- | ------ | ------------------------- |

| POST   | /boards/{boardId}/lists          | Yes  | 201    | Create list               |```

| GET    | /boards/{boardId}/lists          | Yes  | 200    | Get all lists             |POST > http://localhost:8080/workspaces

| GET    | /boards/{boardId}/lists/{listId} | Yes  | 200    | Get list                  |Authorization: Bearer <access_token>

| PUT    | /boards/{boardId}/lists/{listId} | Yes  | 200    | Update list               |```

| DELETE | /boards/{boardId}/lists/{listId} | Yes  | 204    | Delete list and its cards |

```json

### Cards{ "name": "My Workspace" }

```

| Method | Path                                                 | Auth | Status | Description                   |

| ------ | ---------------------------------------------------- | ---- | ------ | ----------------------------- |**Invite a member to the workspace**

| POST   | /boards/{boardId}/lists/{listId}/cards               | Yes  | 201    | Create card                   |

| GET    | /boards/{boardId}/lists/{listId}/cards               | Yes  | 200    | Get all cards                 |```

| GET    | /boards/{boardId}/lists/{listId}/cards/{cardId}      | Yes  | 200    | Get card                      |POST > http://localhost:8080/workspaces/<workspace_id>/members

| PUT    | /boards/{boardId}/lists/{listId}/cards/{cardId}      | Yes  | 200    | Update card                   |Authorization: Bearer <access_token>

| PATCH  | /boards/{boardId}/lists/{listId}/cards/{cardId}/move | Yes  | 200    | Move card to a different list |```

| DELETE | /boards/{boardId}/lists/{listId}/cards/{cardId}      | Yes  | 204    | Delete card                   |

```json

Card status values: `ACTIVE`, `ARCHIVED`, `COMPLETED`.{ "emailOrUsername": "janedoe" }

```

---

**Create a board inside the workspace**

## Usage example

```

**Create a workspace**POST > http://localhost:8080/boards?workspaceId=<workspace_id>

Authorization: Bearer <access_token>

````

POST /workspaces

Authorization: Bearer <access_token>```json

```{ "name": "My Project", "type": "BOARD", "description": "Main project board" }

```

```json

{ "name": "My Workspace" }**Invite a workspace member to a specific board**

```

```

**Invite a member to the workspace**POST > http://localhost:8080/boards/<board_id>/members

Authorization: Bearer <access_token>

```

POST /workspaces/<workspace_id>/members

Authorization: Bearer <access_token>```json

```{ "emailOrUsername": "janedoe" }

```

```json

{ "emailOrUsername": "janedoe" }**Create a list inside the board**

```

```

**Create a board inside the workspace**POST > http://localhost:8080/boards/<board_id>/lists

Authorization: Bearer <access_token>

```

POST /boards?workspaceId=<workspace_id>

Authorization: Bearer <access_token>```json

```{ "name": "To Do" }

```

```json

{ "name": "My Project", "type": "BOARD", "description": "Main project board" }**Create a card inside the list**

```

```

**Invite a workspace member to a specific board**POST > http://localhost:8080/boards/<board_id>/lists/<list_id>/cards

Authorization: Bearer <access_token>

```

POST /boards/<board_id>/members

Authorization: Bearer <access_token>```json

```{ "name": "Task 1", "description": "Task description", "status": "ACTIVE" }

```

```json

{ "emailOrUsername": "janedoe" }---

```

## Error responses

**Create a list inside the board**

All errors follow the exact format:

````

POST /boards/<board_id>/lists```json

Authorization: Bearer <access_token>{

```  "code": "SNAKE_CASE_CODE",

  "message": "Human readable message",

```json  "statusCode": 400,

{ "name": "To Do" }  "timestamp": "2026-03-06T12:00:00Z"

```}

````

**Create a card inside the list**

### Auth errors

````

POST /boards/<board_id>/lists/<list_id>/cards| Code                      | HTTP Status | Endpoint(s)                                                      | Description                                  |

Authorization: Bearer <access_token>| ------------------------- | ----------- | ---------------------------------------------------------------- | -------------------------------------------- |

```| `PASSWORDS_DO_NOT_MATCH`  | 400         | /auth/register                                                   | Passwords do not match                       |

| `EMAIL_ALREADY_EXISTS`    | 400         | /auth/register                                                   | Email already registered                     |

```json| `USERNAME_ALREADY_EXISTS` | 400         | /auth/register                                                   | Username already taken                       |

{ "name": "Task 1", "description": "Task description", "status": "ACTIVE" }| `INVALID_TOKEN`           | 400         | /auth/verify-email                                               | Token not found in database                  |

```| `EXPIRED_TOKEN`           | 400         | /auth/verify-email                                               | Token has passed its 24h expiry              |

| `EMAIL_ALREADY_VERIFIED`  | 400         | /auth/verify-email, /auth/resend-verification                    | Account already active                       |

**Move a card to another list**| `EMAIL_NOT_FOUND`         | 404         | /auth/resend-verification, /auth/forgot-password                 | No account with that email                   |

| `INVALID_CREDENTIALS`     | 401         | /auth/login                                                      | Wrong email/username or password             |

```| `EMAIL_NOT_VERIFIED`      | 403         | /auth/login                                                      | Account pending email verification           |

PATCH /boards/<board_id>/lists/<list_id>/cards/<card_id>/move| `EMAIL_SEND_ERROR`        | 500         | /auth/register, /auth/resend-verification, /auth/forgot-password | AWS SES failure                              |

Authorization: Bearer <access_token>| `VALIDATION_ERROR`        | 400         | Any endpoint with @Valid body                                    | Bean validation failure (see `errors` array) |

````

### Members errors

````json

{ "targetListId": "<target_list_id>", "position": 0 }| Code                    | HTTP Status | Endpoint(s)               | Description                              |

```| ----------------------- | ----------- | ------------------------- | ---------------------------------------- |

| `USER_NOT_FOUND`        | 404         | POST .../members          | No user with that email or username      |

---| `USER_ALREADY_MEMBER`   | 400         | POST .../members          | User is already a member                 |

| `USER_NOT_IN_WORKSPACE` | 400         | POST /boards/{id}/members | User must be a workspace member first    |

## Error responses| `MEMBER_NOT_FOUND`      | 404         | DELETE .../members/{id}   | Member not found                         |

| `CANNOT_REMOVE_OWNER`   | 400         | DELETE .../members/{id}   | The owner cannot be removed              |

All errors follow the format:| `FORBIDDEN`             | 403         | Any members endpoint      | Not a member or insufficient permissions |



```json### Validation error format

{

  "code": "SNAKE_CASE_CODE",```json

  "message": "Human readable message",{

  "statusCode": 400,  "code": "VALIDATION_ERROR",

  "timestamp": "2026-03-06T12:00:00Z"  "message": "Invalid request data.",

}  "statusCode": 400,

```  "errors": [{ "field": "email", "message": "must not be blank" }]

}

### Auth errors```



| Code                      | Status | Endpoint(s)                                                      | Description                                  |---

| ------------------------- | ------ | ---------------------------------------------------------------- | -------------------------------------------- |

| `PASSWORDS_DO_NOT_MATCH`  | 400    | /auth/register                                                   | Passwords do not match                       |_Last updated: March 16, 2026_

| `EMAIL_ALREADY_EXISTS`    | 400    | /auth/register                                                   | Email already registered                     |

| `USERNAME_ALREADY_EXISTS` | 400    | /auth/register                                                   | Username already taken                       |**2 - Run the application**

| `INVALID_TOKEN`           | 400    | /auth/verify-email                                               | Token not found in database                  |

| `EXPIRED_TOKEN`           | 400    | /auth/verify-email                                               | Token has passed its 24h expiry              |Open a terminal in the project root and run:

| `EMAIL_ALREADY_VERIFIED`  | 400    | /auth/verify-email, /auth/resend-verification                    | Account already active                       |

| `EMAIL_NOT_FOUND`         | 404    | /auth/resend-verification, /auth/forgot-password                 | No account with that email                   |```bash

| `INVALID_CREDENTIALS`     | 401    | /auth/login                                                      | Wrong email/username or password             |docker-compose up -d

| `USE_GOOGLE_LOGIN`        | 401    | /auth/login                                                      | Account uses Google Sign-In, no password set |```

| `INVALID_GOOGLE_TOKEN`    | 401    | /auth/google                                                     | Invalid or expired Google ID token           |

| `EMAIL_NOT_VERIFIED`      | 403    | /auth/login                                                      | Account pending email verification           |After running, the following services will be available:

| `EMAIL_SEND_ERROR`        | 500    | /auth/register, /auth/resend-verification, /auth/forgot-password | AWS SES failure                              |

| `VALIDATION_ERROR`        | 400    | Any endpoint with @Valid body                                    | Bean validation failure (see `errors` array) || Service    | Port | URL                                   |

| ---------- | ---- | ------------------------------------- |

### Members errors| API        | 8080 | http://localhost:8080                 |

| Swagger UI | 8080 | http://localhost:8080/swagger-ui.html |

| Code                    | Status | Endpoint(s)               | Description                              || PostgreSQL | 5432 | localhost:5432                        |

| ----------------------- | ------ | ------------------------- | ---------------------------------------- || Redis      | 6379 | localhost:6379                        |

| `USER_NOT_FOUND`        | 404    | POST .../members          | No user with that email or username      |

| `USER_ALREADY_MEMBER`   | 400    | POST .../members          | User is already a member                 |**PostgreSQL connection:**

| `USER_NOT_IN_WORKSPACE` | 400    | POST /boards/{id}/members | User must be a workspace member first    |

| `MEMBER_NOT_FOUND`      | 404    | DELETE .../members/{id}   | Member not found                         |- Host: `localhost`

| `CANNOT_REMOVE_OWNER`   | 400    | DELETE .../members/{id}   | The owner cannot be removed              |- Port: `5432`

| `FORBIDDEN`             | 403    | Any members endpoint      | Not a member or insufficient permissions |- Database: `taskmanager`

- Username: `admin`

### Resource errors- Password: `admin`



| Code                  | Status | Description         |**3 - Configure email sending**

| --------------------- | ------ | ------------------- |

| `WORKSPACE_NOT_FOUND` | 404    | Workspace not found |Open `src/main/resources/application.properties` and set your Gmail credentials:

| `BOARD_NOT_FOUND`     | 404    | Board not found     |

| `LIST_NOT_FOUND`      | 404    | List not found      |```properties

| `CARD_NOT_FOUND`      | 404    | Card not found      |spring.mail.username=your-email@gmail.com

| `FORBIDDEN`           | 403    | Insufficient access |spring.mail.password=your-app-password

| `INVALID_MOVE`        | 400    | Invalid card move   |```



### Validation error formatGenerate a Gmail App Password at:

https://myaccount.google.com/apppasswords

```json

{---

  "code": "VALIDATION_ERROR",

  "message": "Invalid request data.",## Making requests

  "statusCode": 400,

  "errors": [{ "field": "email", "message": "must not be blank" }]Open Swagger UI at http://localhost:8080/swagger-ui.html or use any HTTP client.

}

```All protected endpoints require the header:



---```

Authorization: Bearer <access_token>

## Tech Stack```



| Technology   | Purpose                       |---

| ------------ | ----------------------------- |

| Java 17      | Language                      |## Authentication flow

| Spring Boot  | Framework                     |

| PostgreSQL   | Primary database              |**1 - Register an account**

| Redis        | Refresh token caching         |

| JWT          | Access token authentication   |```

| AWS SES      | Transactional email           |POST > http://localhost:8080/auth/register

| Google OAuth | Social login                  |```

| Docker       | Containerization              |

| Springdoc    | Swagger/OpenAPI documentation |```json

{

---  "name": "John Doe",

  "username": "johndoe",

_Last updated: March 17, 2026_  "email": "john@example.com",

  "password": "secret123",
  "confirmPassword": "secret123"
}
````

Response `201 Created`:

```json
{
  "message": "Registration successful! Please check your email to verify your account."
}
```

A verification email will be sent to the provided address.

**2 - Verify your email**

```
POST > http://localhost:8080/auth/verify-email
```

```json
{ "token": "<token_from_email>" }
```

Response `200 OK`:

```json
{ "message": "Email verified successfully! You can now log in." }
```

**3 - Login**

```
POST > http://localhost:8080/auth/login
```

```json
{ "emailOrUsername": "johndoe", "password": "secret123" }
```

Response `200 OK`:

```json
{
  "name": "John Doe",
  "accessToken": "<access_token>",
  "refreshToken": "<refresh_token>"
}
```

Access token expires in **4 hours**. Use the refresh token to get a new one:

```
POST > http://localhost:8080/auth/refresh
```

```json
{ "refreshToken": "<refresh_token>" }
```

---

## Endpoints

### Auth

| Method | Path                      | Auth | Status (success) | Description                                                |
| ------ | ------------------------- | ---- | ---------------- | ---------------------------------------------------------- |
| POST   | /auth/register            | No   | 201              | Create account and send verification email                 |
| POST   | /auth/verify-email        | No   | 200              | Verify email with token from inbox                         |
| POST   | /auth/resend-verification | No   | 200              | Resend verification email                                  |
| POST   | /auth/login               | No   | 200              | Login, returns access + refresh token                      |
| POST   | /auth/refresh             | No   | 200              | Get new access token using refresh token                   |
| POST   | /auth/logout              | Yes  | 200              | Invalidate current session (requires refreshToken in body) |
| POST   | /auth/logout-all          | Yes  | 200              | Invalidate all sessions                                    |
| POST   | /auth/forgot-password     | No   | 200              | Send password reset email                                  |
| POST   | /auth/reset-password      | No   | 200              | Reset password with token                                  |

### Users

| Method | Path        | Auth | Status (success) | Description                     |
| ------ | ----------- | ---- | ---------------- | ------------------------------- |
| GET    | /users/me   | Yes  | 200              | Get current authenticated user  |
| PUT    | /users/me   | Yes  | 200              | Update profile (name, username) |
| DELETE | /users/me   | Yes  | 204              | Delete own account              |
| GET    | /users/{id} | Yes  | 200              | Get user by ID                  |

### Workspaces

| Method | Path             | Auth | Status (success) | Description                          |
| ------ | ---------------- | ---- | ---------------- | ------------------------------------ |
| POST   | /workspaces      | Yes  | 201              | Create workspace                     |
| GET    | /workspaces      | Yes  | 200              | List all workspaces                  |
| GET    | /workspaces/{id} | Yes  | 200              | Get workspace with boards            |
| PUT    | /workspaces/{id} | Yes  | 200              | Update workspace                     |
| DELETE | /workspaces/{id} | Yes  | 204              | Delete workspace and all its content |

### Boards

| Method | Path                     | Auth | Status (success) | Description                      |
| ------ | ------------------------ | ---- | ---------------- | -------------------------------- |
| POST   | /boards?workspaceId={id} | Yes  | 201              | Create board in workspace        |
| GET    | /boards?workspaceId={id} | Yes  | 200              | List boards from workspace       |
| GET    | /boards/{id}             | Yes  | 200              | Get board with lists and cards   |
| PUT    | /boards/{id}             | Yes  | 200              | Update board                     |
| DELETE | /boards/{id}             | Yes  | 204              | Delete board and all its content |

### Lists

| Method | Path                             | Auth | Status (success) | Description               |
| ------ | -------------------------------- | ---- | ---------------- | ------------------------- |
| POST   | /boards/{boardId}/lists          | Yes  | 201              | Create list               |
| GET    | /boards/{boardId}/lists          | Yes  | 200              | Get all lists             |
| GET    | /boards/{boardId}/lists/{listId} | Yes  | 200              | Get list                  |
| PUT    | /boards/{boardId}/lists/{listId} | Yes  | 200              | Update list               |
| DELETE | /boards/{boardId}/lists/{listId} | Yes  | 204              | Delete list and its cards |

### Cards

| Method | Path                                                 | Auth | Status (success) | Description                   |
| ------ | ---------------------------------------------------- | ---- | ---------------- | ----------------------------- |
| POST   | /boards/{boardId}/lists/{listId}/cards               | Yes  | 201              | Create card                   |
| GET    | /boards/{boardId}/lists/{listId}/cards               | Yes  | 200              | Get all cards                 |
| GET    | /boards/{boardId}/lists/{listId}/cards/{cardId}      | Yes  | 200              | Get card                      |
| PUT    | /boards/{boardId}/lists/{listId}/cards/{cardId}      | Yes  | 200              | Update card                   |
| PATCH  | /boards/{boardId}/lists/{listId}/cards/{cardId}/move | Yes  | 200              | Move card to a different list |
| DELETE | /boards/{boardId}/lists/{listId}/cards/{cardId}      | Yes  | 204              | Delete card                   |

Card status values: `ACTIVE`, `ARCHIVED`, `COMPLETED`.

---

## Usage example

**Create a workspace**

```
POST > http://localhost:8080/workspaces
Authorization: Bearer <access_token>
```

```json
{ "name": "My Workspace" }
```

**Create a board inside the workspace**

```
POST > http://localhost:8080/boards?workspaceId=<workspace_id>
Authorization: Bearer <access_token>
```

```json
{ "name": "My Project", "type": "BOARD", "description": "Main project board" }
```

**Create a list inside the board**

```
POST > http://localhost:8080/boards/<board_id>/lists
Authorization: Bearer <access_token>
```

```json
{ "name": "To Do" }
```

**Create a card inside the list**

```
POST > http://localhost:8080/boards/<board_id>/lists/<list_id>/cards
Authorization: Bearer <access_token>
```

```json
{ "name": "Task 1", "description": "Task description", "status": "ACTIVE" }
```

---

## Error responses

All errors follow the exact format:

```json
{
  "code": "SNAKE_CASE_CODE",
  "message": "Human readable message",
  "statusCode": 400,
  "timestamp": "2026-03-06T12:00:00Z"
}
```

### Auth errors

| Code                      | HTTP Status | Endpoint(s)                                                      | Description                                  |
| ------------------------- | ----------- | ---------------------------------------------------------------- | -------------------------------------------- |
| `PASSWORDS_DO_NOT_MATCH`  | 400         | /auth/register                                                   | Passwords do not match                       |
| `EMAIL_ALREADY_EXISTS`    | 400         | /auth/register                                                   | Email already registered                     |
| `USERNAME_ALREADY_EXISTS` | 400         | /auth/register                                                   | Username already taken                       |
| `INVALID_TOKEN`           | 400         | /auth/verify-email                                               | Token not found in database                  |
| `EXPIRED_TOKEN`           | 400         | /auth/verify-email                                               | Token has passed its 24h expiry              |
| `EMAIL_ALREADY_VERIFIED`  | 400         | /auth/verify-email, /auth/resend-verification                    | Account already active                       |
| `EMAIL_NOT_FOUND`         | 404         | /auth/resend-verification, /auth/forgot-password                 | No account with that email                   |
| `INVALID_CREDENTIALS`     | 401         | /auth/login                                                      | Wrong email/username or password             |
| `EMAIL_NOT_VERIFIED`      | 403         | /auth/login                                                      | Account pending email verification           |
| `EMAIL_SEND_ERROR`        | 500         | /auth/register, /auth/resend-verification, /auth/forgot-password | SMTP failure                                 |
| `EMAIL_AUTH_ERROR`        | 500         | Any email-sending endpoint                                       | SMTP authentication failure                  |
| `VALIDATION_ERROR`        | 400         | Any endpoint with @Valid body                                    | Bean validation failure (see `errors` array) |

### Validation error format

```json
{
  "code": "VALIDATION_ERROR",
  "message": "Invalid request data.",
  "statusCode": 400,
  "errors": [{ "field": "email", "message": "must not be blank" }]
}
```

---

_Last updated: March 6, 2026_

---

## Setup

**1 - Install Docker**
https://www.docker.com

**2 - Run the application**

Open a terminal in the project root and run:

```bash
docker-compose up -d
```

After running, the following services will be available:

| Service    | Port | URL                                   |
| ---------- | ---- | ------------------------------------- |
| API        | 8080 | http://localhost:8080                 |
| Swagger UI | 8080 | http://localhost:8080/swagger-ui.html |
| PostgreSQL | 5432 | localhost:5432                        |
| Redis      | 6379 | localhost:6379                        |

**PostgreSQL connection:**

- Host: `localhost`
- Port: `5432`
- Database: `taskmanager`
- Username: `admin`
- Password: `admin`

**3 - Configure email sending**

Open `src/main/resources/application.properties` and set your Gmail credentials:

```properties
spring.mail.username=your-email@gmail.com
spring.mail.password=your-app-password
```

Generate a Gmail App Password at:
https://myaccount.google.com/apppasswords

---

## Making requests

Open Swagger UI at http://localhost:8080/swagger-ui.html or use any HTTP client.

All protected endpoints require the header:

```
Authorization: Bearer <access_token>
```

---

## Authentication flow

**1 - Register an account**

```
POST > http://localhost:8080/auth/register
```

```json
{
  "name": "John Doe",
  "username": "johndoe",
  "email": "john@example.com",
  "password": "secret123",
  "confirmPassword": "secret123"
}
```

A verification email will be sent to the provided address.

**2 - Verify your email**

```
POST > http://localhost:8080/auth/verify-email
```

```json
{ "token": "<token_from_email>" }
```

**3 - Login**

```
POST > http://localhost:8080/auth/login
```

```json
{ "emailOrUsername": "johndoe", "password": "secret123" }
```

Response:

```json
{
  "name": "John Doe",
  "accessToken": "<access_token>",
  "refreshToken": "<refresh_token>"
}
```

Access token expires in **4 hours**. Use the refresh token to get a new one:

```
POST > http://localhost:8080/auth/refresh
```

```json
{ "refreshToken": "<refresh_token>" }
```

---

## Endpoints

### Auth

| Method | Path                      | Auth | Description                                                |
| ------ | ------------------------- | ---- | ---------------------------------------------------------- |
| POST   | /auth/register            | No   | Register new account                                       |
| POST   | /auth/verify-email        | No   | Verify email with token from inbox                         |
| POST   | /auth/resend-verification | No   | Resend verification email                                  |
| POST   | /auth/login               | No   | Login, returns access + refresh token                      |
| POST   | /auth/refresh             | No   | Get new access token                                       |
| POST   | /auth/logout              | Yes  | Invalidate current session (requires refreshToken in body) |
| POST   | /auth/logout-all          | Yes  | Invalidate all sessions                                    |
| POST   | /auth/forgot-password     | No   | Send password reset email                                  |
| POST   | /auth/reset-password      | No   | Reset password with token                                  |

### Users

| Method | Path        | Auth | Description                     |
| ------ | ----------- | ---- | ------------------------------- |
| GET    | /users/me   | Yes  | Get current authenticated user  |
| PUT    | /users/me   | Yes  | Update profile (name, username) |
| DELETE | /users/me   | Yes  | Delete own account              |
| GET    | /users/{id} | Yes  | Get user by ID                  |

### Workspaces

| Method | Path             | Auth | Description                          |
| ------ | ---------------- | ---- | ------------------------------------ |
| POST   | /workspaces      | Yes  | Create workspace                     |
| GET    | /workspaces      | Yes  | List all workspaces                  |
| GET    | /workspaces/{id} | Yes  | Get workspace with boards            |
| PUT    | /workspaces/{id} | Yes  | Update workspace                     |
| DELETE | /workspaces/{id} | Yes  | Delete workspace and all its content |

### Boards

| Method | Path                     | Auth | Description                      |
| ------ | ------------------------ | ---- | -------------------------------- |
| POST   | /boards?workspaceId={id} | Yes  | Create board in workspace        |
| GET    | /boards?workspaceId={id} | Yes  | List boards from workspace       |
| GET    | /boards/{id}             | Yes  | Get board with lists and cards   |
| PUT    | /boards/{id}             | Yes  | Update board                     |
| DELETE | /boards/{id}             | Yes  | Delete board and all its content |

### Lists

| Method | Path                             | Auth | Description               |
| ------ | -------------------------------- | ---- | ------------------------- |
| POST   | /boards/{boardId}/lists          | Yes  | Create list               |
| GET    | /boards/{boardId}/lists          | Yes  | Get all lists             |
| GET    | /boards/{boardId}/lists/{listId} | Yes  | Get list                  |
| PUT    | /boards/{boardId}/lists/{listId} | Yes  | Update list               |
| DELETE | /boards/{boardId}/lists/{listId} | Yes  | Delete list and its cards |

### Cards

| Method | Path                                                 | Auth | Description                   |
| ------ | ---------------------------------------------------- | ---- | ----------------------------- |
| POST   | /boards/{boardId}/lists/{listId}/cards               | Yes  | Create card                   |
| GET    | /boards/{boardId}/lists/{listId}/cards               | Yes  | Get all cards                 |
| GET    | /boards/{boardId}/lists/{listId}/cards/{cardId}      | Yes  | Get card                      |
| PUT    | /boards/{boardId}/lists/{listId}/cards/{cardId}      | Yes  | Update card                   |
| PATCH  | /boards/{boardId}/lists/{listId}/cards/{cardId}/move | Yes  | Move card to a different list |
| DELETE | /boards/{boardId}/lists/{listId}/cards/{cardId}      | Yes  | Delete card                   |

Card status values: `ACTIVE`, `ARCHIVED`, `COMPLETED`.

---

## Usage example

**Create a workspace**

```
POST > http://localhost:8080/workspaces
Authorization: Bearer <access_token>
```

```json
{ "name": "My Workspace" }
```

**Create a board inside the workspace**

```
POST > http://localhost:8080/boards?workspaceId=<workspace_id>
Authorization: Bearer <access_token>
```

```json
{ "name": "My Project", "type": "BOARD", "description": "Main project board" }
```

**Create a list inside the board**

```
POST > http://localhost:8080/boards/<board_id>/lists
Authorization: Bearer <access_token>
```

```json
{ "name": "To Do" }
```

**Create a card inside the list**

```
POST > http://localhost:8080/boards/<board_id>/lists/<list_id>/cards
Authorization: Bearer <access_token>
```

```json
{ "name": "Task 1", "description": "Task description", "status": "ACTIVE" }
```

---

## Error responses

All errors follow the format:

```json
{ "code": "ERROR_CODE", "message": "Human readable message." }
```

| Code                   | Status | Description                        |
| ---------------------- | ------ | ---------------------------------- |
| EMAIL_NOT_VERIFIED     | 403    | Account pending email verification |
| INVALID_CREDENTIALS    | 400    | Wrong email/username or password   |
| EMAIL_NOT_FOUND        | 400    | No account with that email         |
| EMAIL_ALREADY_VERIFIED | 400    | Account already verified           |

---

_Last updated: March 6, 2026_
